...
int camera_read_frame(void){
	MMAL_BUFFER_HEADER_T* buf = mmal_queue_get(video_queue);
	if(buf){
	...
	return 0; //no buffer received
}

void create_splitter_connections(MMAL_PORT_T *splitter_port)
{
	int width = 1920;
	int height = 1080;
	int frame_rate = 30;
	...
	{
		printf("Couldn't set splitter port format : error %d\n", status);
	}

	status = mmal_port_parameter_set_boolean(splitter_port,
	MMAL_PARAMETER_ZERO_COPY, MMAL_TRUE);
	if (status != MMAL_SUCCESS)
	{
		printf("Failed to enable zero copy on splitter port\n");
		exit(1);
	}
	...
	*/
	splitter_port->buffer_num = 3;
	splitter_port->buffer_size = splitter_port->buffer_size_recommended;


	/* Pool + queue to hold preview frames */
	video_pool = mmal_port_pool_create(splitter_port,splitter_port->buffer_num,splitter_port->buffer_size);
	if (!video_pool)
	{
		printf("Error allocating camera video pool. Buffer num: %d Buffer size: %d\n", splitter_port->buffer_num, splitter_port->buffer_size);
		status = MMAL_ENOMEM;
		exit(1);
	}
	...
	status = mmal_port_enable(splitter_port, video_output_callback);
	if (status != MMAL_SUCCESS)
	{
	...
	//send all the buffers in our pool to the video port ready for use
	{
		...
		for (q=0;q<num;q++)
		{
			...
			else if (mmal_port_send_buffer(splitter_port, buffer)!= MMAL_SUCCESS)
			{
...
void video_output_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer){
	...
	{
		MMAL_BUFFER_HEADER_T* existing_buffer = mmal_queue_get(video_queue);
		if(existing_buffer)
		{
